...
//        only authorized user can Get All blogs Post
    ...
    @GetMapping("blogs/postId/pagination")
    public List<Comment> getCommentsByPostId(@RequestParam Integer postId,  @RequestParam @Valid String email, @RequestParam String token, @RequestParam(defaultValue = "0") Integer page, @RequestParam Integer pageSize) {
        boolean authenticateResult = authenticationService.authenticate(email, token);
        if(authenticateResult) {
            List<Comment> commentList = userService.getCommentsByPostId(postId, page, pageSize);
            return commentList;
        }
        else {
            throw new RuntimeException("Not an Authenticated user activity!!!");
